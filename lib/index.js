...
function reset_page() {
  $("#main").html(`<div class='search'>
  <h2>Search for Weather!</h2>
  <input class='input-field' type='text' placeholder='Ex: New York, NY' id='location'>
  <input id='search' class='btn' type='submit' value='Search'></div>`);
}
...
function current(data, local) {
  ...
  + "<div class='location-data'><h2 id='local' class='local'>" + local
  + "</h2><p class='country'>" + data.timezone + "</p><p>"
  + get_time_and_date(data.currently.time)
  + "</p></div></section>";
}
...
function details(data, local) {
  ...
  + "</h3></div><div class='weather-details'><p>Feels Like: "
...
function user_register() {
  ...
  var url = "https://shielded-tor-51189.herokuapp.com/api/v1/users";
  $.post(url, function(data) {
    document.cookie = data.api_key;
  }).done(function() {
    toggle_log_in_out();
  })
  .fail(function() {
...
function user_favorite() {
  var local = $("#local").html();
  var data = JSON.stringify({
    "location": local,
    "api_key": document.cookie
  });
  var url = "https://shielded-tor-51189.herokuapp.com/api/v1/favorites";
  debugger;
  $.post(url, function(data) {
    debugger;
  }).fail(function() {
...
$(document).ready( function() {
  ...
  $("#favorite").click(user_favorite);

  $("#register").click(user_register);

  $("#logout").click(user_logout);

  $("#search").click(search_location);

  $("#home").click(reset_page);
