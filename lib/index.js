...
function current(data, local) {
  ...
  return "<section class='current'><div class='icon weather-" + data.currently.icon + "'><p>" + data.currently.summary + "</p></div><h3 class='temp'>" + data.currently.temperature + "&deg;</h3>" + location_string + "<p>" + get_time_and_date(data.currently.time) + "</p></section>";
}

function details(data, local) {
  return "<section class='details'><div class='icon weather-" + data.currently.icon + "'><h3>" + data.currently.summary + "</h3></div><p>Feels Like: " + data.currently.apparentTemperature + "&deg;</p><p>Humidity: " + percentage(data.currently.humidity) + "%</p><p>Visibility: " + data.currently.visibility + " miles</p><p>UV Index: "+ data.currently.uvIndex + "</p></section>";
}
...
function daily_structure(v) {
  return "<div class='day'><h4>" + get_day(v.time) + "</h4><div class='icon weather-" + v.icon + "'><p>" + v.summary + "</p></div><div class='icon precip-" + v.precipType + "'><p>" + percentage(v.precipProbability) + "%</p></div><div class='icon high'><p>" + v.temperatureHigh + "&deg;</p></div><div class='icon low'><p>" + v.temperatureLow + "&deg;</p></div></div>";
}
